...
namespace Fx {
	...
	public:
		...
		virtual T total_dx() //for linear movement
		{
			return repeat_index * delta_data + dx();
		}
	...
	private:
		...
		virtual T calculate( float t1, float t2 )
		{
			/*
			switch( trans ) {
...
public:
	...
	/*
		Syntax Documentation:
			...
			Effect_t::Animate( float& from, float to, transition_t trans, float duration, float delay, int repeat );
...
int main(){
	...
	Fx -> Animate( y, 1000.f, Fx::CONSTANT_ANIMATION, 1.f, .5f, -1 );
