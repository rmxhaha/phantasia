...
world_map::world_map( int w, int h ){
...
int world_map::find_enemy( int x, int y ){
	return find_enemy( y*map_width + x );
}

int world_map::find_enemy( int index ){
...
bool world_map::check_exist( int index ){
	return ( find_enemy( index ) >= 0 );
}

bool world_map::check_exist( int index, int& eindex ){
...
void world_map::move_hero( char direction ) {
	char dx = 0, dy = 0;
...
void world_map::handle_enemy(){

}

void world_map::encounter( enemy_struct& enemy, bool plus ){
	if( plus ) cout << "Speed+ "; else cout << "Encounter ";

	cout << enemy.type -> name << endl;
}

void world_map::add_enemy( enemy_struct input ){
	enemies.push_back( input );
}

void world_map::add_random_enemy( int number ){
...
void world_map::add_random_wall( int number ){
	int i = map_height*map_width;
	int rx,ry;
